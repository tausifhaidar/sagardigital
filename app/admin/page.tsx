...
export default function AdminPage() {
  ...
  const [email, setEmail] = useState("tausifhaidar63@gmail.com");
  ...
  async function load() {
    setBusy(true);
    const [o, q, f] = await Promise.all([
      supabase.from("orders").select("*").order("created_at", { ascending: false }).limit(100),
      supabase.from("quote_requests").select("*").order("created_at", { ascending: false }).limit(100),
      supabase.from("order_files").select("*").order("created_at", { ascending: false }).limit(100),
    ]);
    setOrders(o.data || []); setQuotes(q.data || []); setFiles(f.data || []);
    if (o.error || q.error || f.error) setNotice("Some admin data could not be loaded.");
    setBusy(false);
  }
  ...
  async function changeOrderStatus(order: any, status: string) {
    const { error } = await supabase.from("orders").update({ status }).eq("id", order.id);
    if (error) { setNotice(error.message); return; }
    await supabase.from("order_status_history").insert({ order_id: order.id, status, note: `Admin changed status to ${statusLabels[status]}` });
    setNotice(`${order.order_number} updated.`); load();
  }

  async function changePayment(order: any, payment_status: string) {
    const { error } = await supabase.from("orders").update({ payment_status }).eq("id", order.id);
    setNotice(error ? error.message : `${order.order_number} payment updated.`); if (!error) load();
  }

  async function changeQuote(quote: any, status: string) {
    const { error } = await supabase.from("quote_requests").update({ status }).eq("id", quote.id);
    setNotice(error ? error.message : `${quote.quote_number} updated.`); if (!error) load();
  }

  const customers = useMemo(() => {
    const map = new Map<string, any>();
    orders.forEach(o => map.set(o.customer_phone, { name: o.customer_name, phone: o.customer_phone, count: (map.get(o.customer_phone)?.count || 0) + 1 }));
    return [...map.values()];
  }, [orders]);
  ...
  return <main className="min-h-screen bg-slate-100">
    <header className="bg-slate-950 text-white"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-5 py-5"><div><p className="text-xs font-bold uppercase tracking-widest text-red-400">Sagar Digital</p><h1 className="text-2xl font-black">Admin Dashboard</h1></div><div className="flex gap-3"><span className="hidden rounded-xl border border-white/10 px-4 py-2 text-sm sm:block">{session.user.email}</span><button onClick={()=>supabase.auth.signOut()} className="rounded-xl bg-white px-4 py-2 text-sm font-bold text-slate-900">Logout</button></div></div></header>
    <div className="mx-auto max-w-7xl px-5 py-7">
      <div className="flex gap-2 overflow-x-auto rounded-2xl border bg-white p-2">{tabs.map(t=><button key={t} onClick={()=>setTab(t)} className={`whitespace-nowrap rounded-xl px-4 py-2.5 text-sm font-bold ${tab===t ? "bg-red-600 text-white" : "text-slate-600 hover:bg-slate-100"}`}>{t}</button>)}</div>
      ...
      {busy ? <div className="mt-8 rounded-2xl bg-white p-8 text-center">Loading...</div> : <div className="mt-6">
        ...
        {tab === "Files" && <div className="space-y-4">{files.map(f=><div key={f.id} className="rounded-2xl border bg-white p-5"><b>{f.original_name || "Uploaded file"}</b><p className="text-sm text-slate-500">{f.mime_type || "Unknown"} · {f.size_bytes ? `${Math.round(f.size_bytes/1024)} KB` : ""}</p><p className="mt-2 break-all text-xs text-slate-400">{f.storage_path}</p></div>)}</div>}
      </div>}
    </div>
  </main>;
}
...
function OrderCard({order,onStatus,onPayment,detailed=false}:{order:any;onStatus:(o:any,s:string)=>void;onPayment?:(o:any,s:string)=>void;detailed?:boolean}){return <div className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex flex-wrap justify-between gap-4"><div><b>{order.order_number}</b><p className="text-sm text-slate-600">{order.customer_name} · {order.customer_phone}</p><p className="mt-2 text-sm">{order.service || order.product || "Order"}{order.quantity ? ` · Qty ${order.quantity}` : ""}</p>{detailed&&<p className="mt-2 text-sm text-slate-500">{order.requirement || "No requirement note"}</p>}</div><div className="flex flex-wrap gap-2"><select value={order.status} onChange={e=>onStatus(order,e.target.value)} className="rounded-xl border px-3 py-2 text-sm font-bold">{statuses.map(s=><option key={s} value={s}>{statusLabels[s]}</option>)}</select>{onPayment&&<select value={order.payment_status} onChange={e=>onPayment(order,e.target.value)} className="rounded-xl border px-3 py-2 text-sm font-bold"><option value="pending">Payment pending</option><option value="partial">Partial</option><option value="paid">Paid</option><option value="refunded">Refunded</option></select>}</div></div></div>}
