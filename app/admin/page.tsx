  ...
  return <main className="min-h-screen bg-slate-100">
    ...
    <div className="mx-auto max-w-7xl px-5 py-7"><div className="flex gap-2 overflow-x-auto rounded-2xl border bg-white p-2">{tabs.map(t=><button key={t} onClick={()=>setTab(t)} className={`whitespace-nowrap rounded-xl px-4 py-2.5 text-sm font-bold ${tab===t ? "bg-red-600 text-white" : "text-slate-600 hover:bg-slate-100"}`}>{t}</button>)}</div>
      ...
      {busy ? <div className="mt-8 rounded-2xl bg-white p-8 text-center">Loading...</div> : <div className="mt-6">
        ...
        {tab === "Payments" && <div className="space-y-4">{orders.map(o=><div key={o.id} className="rounded-2xl border bg-white p-5"><div className="flex flex-wrap justify-between gap-4"><div><b>{o.order_number}</b><p className="text-sm text-slate-500">Total ₹{Number(o.total_amount||0).toLocaleString("en-IN")} · Advance ₹{Number(o.advance_amount||0).toLocaleString("en-IN")}</p><p className="mt-1 text-xs text-slate-400">Ordered: {formatDateTime(o.created_at)}</p></div><select value={o.payment_status} onChange={e=>changePayment(o,e.target.value)} className="rounded-xl border px-3 py-2 text-sm font-bold"><option value="pending">Pending</option><option value="partial">Partial</option><option value="paid">Paid</option><option value="refunded">Refunded</option></select></div></div>)}</div>}
...
function OrderCard({order,onStatus,onPayment,detailed=false}:{order:any;onStatus:(o:any,s:string)=>void;onPayment?:(o:any,s:string)=>void;detailed?:boolean}){return <div className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex flex-wrap justify-between gap-4"><div className="min-w-0"><div className="flex flex-wrap items-center gap-3"><b>{order.order_number}</b><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{formatDateTime(order.created_at)}</span></div><p className="text-sm text-slate-600">{order.customer_name} · {order.customer_phone}</p><p className="mt-2 text-sm">{order.service || order.product || "Order"}{order.quantity ? ` · Qty ${order.quantity}` : ""}</p>{detailed&&<><p className="mt-2 text-sm text-slate-500">{order.requirement || "No requirement note"}</p><div className="mt-3 rounded-xl bg-slate-50 p-3 text-sm"><b>{order.delivery_method === "delivery" ? "🛵 Home delivery" : "🏪 Shop pickup"}</b>{order.delivery_method === "delivery" && <><span className="ml-2 text-slate-600">₹{Number(order.delivery_charge||0)} delivery</span>{order.delivery_distance_km && <span className="ml-2 text-slate-500">· {order.delivery_distance_km} km</span>}{order.delivery_address && <p className="mt-1 text-slate-600">{order.delivery_address}</p>}</>}</div></>}</div><div className="flex flex-wrap gap-2"><select value={order.status} onChange={e=>onStatus(order,e.target.value)} className="rounded-xl border px-3 py-2 text-sm font-bold">{statuses.map(s=><option key={s} value={s}>{statusLabels[s]}</option>)}</select>{onPayment&&<select value={order.payment_status} onChange={e=>onPayment(order,e.target.value)} className="rounded-xl border px-3 py-2 text-sm font-bold"><option value="pending">Payment pending</option><option value="partial">Partial</option><option value="paid">Paid</option><option value="refunded">Refunded</option></select>}</div></div></div>}
function FilesList({files,orders}:{files:any[];orders:any[]}){const [loading,setLoading]=useState<string>(""); async function openFile(f:any){setLoading(f.id); const {data,error}=await supabase.storage.from("order-files").createSignedUrl(f.storage_path,3600); setLoading(""); if(error||!data?.signedUrl){alert("File could not be opened.");return;} window.open(data.signedUrl,"_blank","noopener,noreferrer");} if(!files.length) return <div className="rounded-2xl border bg-white p-8 text-center"><p className="text-lg font-black text-slate-800">No uploaded files yet</p><p className="mt-2 text-sm text-slate-500">Files attached to new orders will appear here with the Order ID and upload time.</p></div>; return <div className="space-y-4">{files.map(f=>{const order=orders.find(o=>o.id===f.order_id);return <div key={f.id} className="rounded-2xl border bg-white p-5"><div className="flex flex-wrap items-center justify-between gap-4"><div><b>{f.original_name||"Uploaded file"}</b><p className="text-sm text-slate-500">{order?.order_number || "Order file"} · {order?.customer_name || ""}</p><p className="text-sm text-slate-500">{f.mime_type||"Unknown"} · {f.size_bytes?`${Math.round(f.size_bytes/1024)} KB`:""}</p><p className="mt-1 text-xs text-slate-400">Uploaded: {formatDateTime(f.created_at)}</p></div><button onClick={()=>openFile(f)} disabled={loading===f.id} className="rounded-xl bg-red-600 px-4 py-2 text-sm font-bold text-white">{loading===f.id?"Opening...":"Open file"}</button></div></div>})}</div>}
