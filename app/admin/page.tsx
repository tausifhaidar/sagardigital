...
export default function AdminPage() {
  ...
  async function changeOrderStatus(order: any, status: string) {
    ...
    setNotice(error ? error.message : `${order.order_number} updated.`); if (!error) load();
  }
  async function changePayment(order: any, payment_status: string) {
    const { error } = await supabase.rpc("admin_update_payment_status", { p_order_id: order.id, p_payment_status: payment_status });
  ...
  async function changeQuote(quote: any, status: string) {
    const { error } = await supabase.rpc("admin_update_quote_status", { p_quote_id: quote.id, p_status: status });
